...
fn main() {
  ...
  match matches.subcommand() {
    ...
    Some(("gen-metal", sub_matches)) => {
      ...
      book.to_buffer(&mut book_buf);
      let bookb = format!("{:?}", book_buf).replace("[","{").replace("]","}");
      let bookb = format!("static const unsigned char BOOK_BUF[] = {};", bookb);

      // Embeds shader source for runtime compilation in the generated standalone file.
      let metal_src = include_str!("hvm.metal")
        .replace('\\', r#"\\"#)
        .replace('\"', "\\\"")
        .replace('\n', "\\n\"\n\"");

      let runtime = include_str!("run.metal.mm");
      let out = format!(
        "// Generated by `hvm gen-metal`.\n\
         // Build with:\n\
         // clang++ -x objective-c++ -std=c++17 -O3 -fobjc-arc generated.mm -framework Foundation -framework Metal -o generated\n\n\
         #define HVM_GEN_STANDALONE 1\n\n\
